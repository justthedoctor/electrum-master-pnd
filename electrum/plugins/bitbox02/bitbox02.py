...
class BitBox02Client(HardwareClientBase):
    ...
    def coin_network_from_electrum_network(self) -> int:
        if constants.net.TESTNET:
            return bitbox02.btc.TPND
        return bitbox02.btc.PND
    ...
    def get_xpub(self, bip32_path: str, xtype: str, *, display: bool = False) -> str:
        ...
        if xtype == "p2wpkh":
            if coin_network == bitbox02.btc.PND:
                out_type = bitbox02.btc.PNDPubRequest.ZPUB
            else:
                out_type = bitbox02.btc.PNDPubRequest.VPUB
        elif xtype == "p2wpkh-p2sh":
            if coin_network == bitbox02.btc.PND:
                out_type = bitbox02.btc.PNDPubRequest.YPUB
            else:
                out_type = bitbox02.btc.PNDPubRequest.UPUB
        elif xtype == "p2wsh-p2sh":
            if coin_network == bitbox02.btc.PND:
                out_type = bitbox02.btc.PNDPubRequest.CAPITAL_YPUB
            else:
                out_type = bitbox02.btc.PNDPubRequest.CAPITAL_UPUB
        elif xtype == "p2wsh":
            if coin_network == bitbox02.btc.PND:
                out_type = bitbox02.btc.PNDPubRequest.CAPITAL_ZPUB
            else:
                out_type = bitbox02.btc.PNDPubRequest.CAPITAL_VPUB
    ...
    def btc_multisig_config(
        ...
        multisig_config = bitbox02.btc.PNDScriptConfig(
            multisig=bitbox02.btc.PNDScriptConfig.Multisig(
                threshold=wallet.m,
                xpubs=[util.parse_xpub(xpub) for xpub in xpubs],
                our_xpub_index=xpubs.index(our_xpub),
                script_type={
                    "p2wsh": bitbox02.btc.PNDScriptConfig.Multisig.P2WSH,
                    "p2wsh-p2sh": bitbox02.btc.PNDScriptConfig.Multisig.P2WSH_P2SH,
                }[xtype]
            )
        )
    ...
    def show_address(
        ...
        if address_type == "p2wpkh":
            script_config = bitbox02.btc.PNDScriptConfig(
                simple_type=bitbox02.btc.PNDScriptConfig.P2WPKH
            )
        elif address_type == "p2wpkh-p2sh":
            script_config = bitbox02.btc.PNDScriptConfig(
                simple_type=bitbox02.btc.PNDScriptConfig.P2WPKH_P2SH
            )
    ...
    def _get_coin(self):
        return bitbox02.btc.TPND if constants.net.TESTNET else bitbox02.btc.PND
    ...
    def sign_transaction(
        ...
        if tx_script_type == "p2wpkh":
            tx_script_type = bitbox02.btc.PNDScriptConfig(
                simple_type=bitbox02.btc.PNDScriptConfig.P2WPKH
            )
        ...
        sigs = self.bitbox02_device.btc_sign(
            coin,
            [bitbox02.btc.PNDScriptConfigWithKeypath(
    ...
    def sign_message(self, keypath: str, message: bytes, script_type: str) -> bytes:
        ...
        try:
            simple_type = {
                "p2wpkh-p2sh":bitbox02.btc.PNDScriptConfig.P2WPKH_P2SH,
                "p2wpkh": bitbox02.btc.PNDScriptConfig.P2WPKH,
            }[script_type]
        except KeyError:
